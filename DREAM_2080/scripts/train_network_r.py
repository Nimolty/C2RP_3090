...
#            "batch_training_loss_pred":[],
    ...
    print('enable_augment_data', enable_augment_data)
...
#        f.write(str(train_dataset) + '\n')
    ...
    tb_path = f'/mnt/data/Dream_ty/Dream_model/ty_logs/{args.name}/tb'
    print(tb_path)
...
#                f.write(str(sample) + '\n')
            ...
            writer.add_scalar(f' train_loss', loss, batch_idx + e * train_data_loader_length)
            writer.add_scalar(f' loss_1', loss_1, batch_idx + e * train_data_loader_length)
            writer.add_scalar(f' loss_2', loss_2, batch_idx + e * train_data_loader_length)
...
#                print('loss_pred', loss_pred)
            ...
            if batch_idx % 50 == 0:
                with torch.no_grad():
                    dream_network.add_heatmap(network_input_heads, training_labels, writer, batch_idx, e, train_data_loader_length)

            training_loss_this_batch = loss.item()
            training_loss_1_this_batch = loss_1
            training_loss_2_this_batch = loss_2
...
#                os.remove(last_epoch_training_log_path)
        ...
        if args.infer_per_epoch:
            with torch.no_grad():
                epoch_infer_output_path = os.path.join(infer_output_dir, "inference_res_{}".format(this_epoch)
                )
                input_param_path = os.path.join(args.output_dir, "epoch_{}".format(this_epoch) + '.pth')
                input_config_path = os.path.join(args.output_dir, "epoch_{}".format(this_epoch) + ".yaml")
                visualize_belief_maps = not args.not_visualize_belief_maps
                dream_analysis.analyze_ndds_dataset(
                    input_param_path,
                    input_config_path,
                    args.infer_data_path,
...
#        nargs="+",
    ...
    parser.add_argument(
        ...
        "--infer-per-epoch",
        default=False,
        help="Records the inference statistics after each epoch"
    )
    ...
    parser.add_argument(
        ...
        "--infer-data-path",
        type=str,
        default="/Huge_Disk/Dream_data_all/data/real/panda-3cam_realsense/",
        help="input inference data path"
    )
...
#        "--infer-output-dir",
#        type=str,
#        default="/mnt/data/Dream_data_all/data/real/panda-3cam_realsense/",
#        help="input inference data path"
#    )
