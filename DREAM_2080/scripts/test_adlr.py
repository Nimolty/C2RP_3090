...
import dream
from dream.utilities import exists_or_mkdir
...
def train_network(args):
    ...
    if args.resume_training:
        ...
        epoch_weight_paths = [x[0] for x in temp][::-1]
        epoch_numbers = [x[1] for x in temp][::-1]
        
        if args.resume_training_target:
            # specify a certain network 
            epoch_weight_path = epoch_weight_paths[args.resume_training_target-1]
            start_epoch = epoch_numbers[args.resume_training_target-1]
        else:
            # else specify the most recent network
            epoch_weight_path = epoch_weight_paths[-1]
            start_epoch = epoch_numbers[-1]
        ...
        if os.path.exists(os.path.join(args.output_dir, "training_log.pkl")):
            ...
            os.rename(
        ...
        elif os.path.exists(os.path.join(args.output_dir, "training_log_e{}.pkl".format(start_epoch))):
             ...
             train_log = {
                 ...
                 "batch_training_loss_1" : [],
                 "batch_training_loss_2" : [],
...
#            "batch_training_loss_r" : [],
    ...
    camera_mat = dream.utilities.load_camera_intrinsics(found_data_config["camera"])
    print("AAAAAAAAAAAAAAAAAAAA", camera_mat)
    ...
    if args.resume_training:
        ...
        epoch_config_path = epoch_weight_path.replace("pth", "yaml")
        config_parser = YAML(typ="safe")

        with open(os.path.join(args.output_dir, epoch_config_path), "r") as f:
            epoch_network_config_file = config_parser.load(f)

        # Do a bunch of network consistency checks
        assert (
                epoch_network_config_file["data_path"] == network_config["data_path"]
        )
        assert (
                epoch_network_config_file["manipulator"]
                == network_config["manipulator"]
        )
        assert (
                epoch_network_config_file["architecture"]
                == network_config["architecture"]
        )
        assert (
                epoch_network_config_file["training"]["config"][
                    "training_data_fraction"
                ]
        ...
        assert (
                epoch_network_config_file["training"]["config"]["batch_size"]
                == network_config["training"]["config"]["batch_size"]
        )
        assert (
                epoch_network_config_file["training"]["config"]["data_augmentation"]
                == network_config["training"]["config"]["data_augmentation"]
        )
        assert (
                epoch_network_config_file["training"]["config"]["worker_size"]
                == network_config["training"]["config"]["worker_size"]
        )
        assert (
                epoch_network_config_file["training"]["config"]["optimizer"]
                == network_config["training"]["config"]["optimizer"]
        )
        assert (
                epoch_network_config_file["training"]["config"]["image_preprocessing"]
        ...
        network_config = epoch_network_config_file
        if args.resume_training_target:
            adaptive_lr = args.adapt_learning_rate
            network_config['training']['config']['optimizer']['learning_rate'] = adaptive_lr

        print("~~ RESUMING TRAINING FROM {} ~~".format(epoch_weight_path))
        print("")
    ...
    dream_network = dream.network.create_network_from_config_data(network_config)
    if args.resume_training:
        dream_network.model.load_state_dict(
            torch.load(os.path.join(args.output_dir, epoch_weight_path))
        )
        print('#################')
        print(epoch_weight_path)
        print('#################')
    ...
    tb_path = f'/Huge_Disk/Dream_data_0809/Dream_model/ty_logs/{args.name}/tb'
    print(tb_path)
...
#    if args.resume_training:
#        prev_loss = np.concatenate(train_log["batch_training_losses"], axis=0)
##        prev_loss_1 = np.concatenate(train_log["batch_training_loss_1"], axis=0)
##        prev_loss_2 = np.concatenate(train_log["batch_training_loss_2"], axis=0)
...
#        length = prev_loss.shape[0]
#        for i in range(length):
#            writer.add_scalar(f' train_loss', prev_loss[i], i)
##            writer.add_scalar(f' loss_1', prev_loss_1[i], i)
##            writer.add_scalar(f' loss_2', prev_loss_2[i], i)
...
##            writer.add_scalar(f' loss_r', prev_loss_r[i], i)
    ...
    for e in tqdm(range(start_epoch, args.epochs)):
        ...
        training_batch_loss_1 = []
        training_batch_loss_2 = []
        ...
        for batch_idx, sample in enumerate(tqdm(train_data_loader)):
            ...
            network_input_heads.append(sample["keypoint_positions"].cuda())
            ...
            if dream_network.network_config["architecture"]["target"] == "belief_maps":
                ...
                print("training:", network_input_heads.shape)

            
            #print('training_size:', training_labels.shape)
            #print(training_labels.shape, training_labels.max(), training_labels.min())
            ###################################
            ...
            loss, loss_1, loss_2, loss_mc, loss_t, loss_r = dream_network.train(network_input_heads, pose_gt,camera_mat,training_labels,this_epoch, dream_network.image_preprocessing())
            ###################################
            
            writer.add_scalar(f' train_loss', loss, batch_idx + e * len(train_data_loader))
            writer.add_scalar(f' loss_1', loss_1, batch_idx + e * len(train_data_loader))
            writer.add_scalar(f' loss_2', loss_2, batch_idx + e * len(train_data_loader))
            writer.add_scalar(f' loss_mc', loss_mc, batch_idx + e * len(train_data_loader))
            writer.add_scalar(f' loss_t', loss_t, batch_idx + e * len(train_data_loader))
            writer.add_scalar(f' loss_r', loss_r, batch_idx + e * len(train_data_loader))
            
#            for name, param in dream_network.named_parameters():
#                print(name, param)

#            print(dream_network.parameters())
            if (batch_idx % 10 == 0):
                ...
                print('loss_r', loss_r.item())

            training_loss_this_batch = loss.item()
            training_loss_1_this_batch = loss_1.item()
            training_loss_2_this_batch = loss_2.item()
            training_loss_mc_this_batch = loss_mc.item()
            training_loss_t_this_batch = loss_t.item()
            training_loss_r_this_batch = loss_r.item()
        ...
        with torch.no_grad():
            ...
            for valid_batch_idx, valid_sample in enumerate(tqdm(valid_data_loader)):
                ...
                valid_network_input_heads.append(valid_sample["keypoint_positions"].cuda())
                ...
                valid_loss = dream_network.loss(valid_network_input_heads, valid_pose_gt, camera_mat, valid_labels, this_epoch , 1)
        ...
        if save_results:
            ...
            # Don't remove old training_log, since we may use it to detect errors.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=5e-5,
        help="The adaptive learning rate used for the opimizer.",
    )
    ...
    parser.add_argument(
        "-rt",
        "--resume-training-target",
        type = int,
        default = 0,
        help="Resumes training target. The epoch argument provided now is the target training duration. All arguments must match the previously trained networks."
    )
    parser.add_argument(
        "-n",
        "--name",
