...
def project_a(x3d, pose, cam_mats, z_min: float):
    ...
    # print("aaa",x3d_rot.shape,pose[..., None, :3].shape,(x3d_rot + pose[..., None, :3]).shape,cam_mats.transpose(-1, -2).shape)
    x2dh_proj = (x3d_rot + pose[..., None, :3]) @ cam_mats.transpose(-1, -2).repeat((int(x3d_rot.shape[0]/cam_mats.shape[0]),1,1))
    # print("??????")
...
class PerspectiveCamera(object):
    ...
    def project_jacobian(self, x3d_rot, zcam, x2d_proj, out_jac, dof):
        ...
        if zcam.requires_grad or x2d_proj.requires_grad:
            ...
            # print('test',x3d_rot.shape)
            jac[..., :2] = self.cam_mats[..., None, :2, :2].repeat((int(x3d_rot.shape[0]/self.cam_mats.shape[0]),1,1,1)) / zcam.unsqueeze(-1)
            # d_x2d_d_zcam (b, n, 2, 1) = ((b, 1, 2, 1) - (b, n, 2, 1)) / (b, n, 1, 1)
            jac[..., 2:3] = (self.cam_mats[..., None, :2, 2:3].repeat((int(x3d_rot.shape[0]/self.cam_mats.shape[0]),1,1,1)) - x2d_proj.unsqueeze(-1)
                             ) / zcam.unsqueeze(-1)
