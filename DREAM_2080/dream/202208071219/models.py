...
from .spatial_softmax import SoftArgmax
...
class DreamHourglass(nn.Module):
    def __init__(
        ...
        self.heads_1.add_module(
            "0", nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
        )
        self.heads_1.add_module("1", nn.ReLU(inplace=True))
        self.heads_1.add_module(
            "2", nn.Conv2d(256, 64, kernel_size=3, stride=1, padding=1)
        )
        self.heads_1.add_module("3", nn.ReLU(inplace=True))
        self.heads_1.add_module(
            "4", nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)
        )
        self.heads_1.add_module("5", nn.ReLU(inplace=True))
        self.heads_1.add_module(
            "6", nn.Conv2d(32, self.n_keypoints, kernel_size=3, stride=1, padding=1)
        )
