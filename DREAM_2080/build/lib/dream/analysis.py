...
#    all_rgb = []
    ...
    with torch.no_grad():
        ...
        for batch_idx, sample in enumerate(tqdm(data_loader)):
            ...
            _, detected_keypoints_netout_batch, _ = dream_network.model(
                network_image_input
            )
            detected_keypoints_netout_batch = detected_keypoints_netout_batch.cpu().numpy()
            ...
            for b in range(this_batch_size):
                ...
                #print(this_detected_kps_netout)
                #print(network_output_res_inf, network_input_res_inf)
                this_detected_kps_netin = dream.image_proc.convert_keypoints_to_netin_from_netout(
                    this_detected_kps_netout,
                    network_output_res_inf,
                    network_input_res_inf,
                )
                #print(image_raw_resolution, network_input_res_inf)
...
#                    best_net_input_images_viz_batch = sample["image_rgb_input_viz"]
                    network_output_heads = dream_network.model(network_image_input)
                    best_samples_belief_maps = network_output_heads[0]
                    best_detected_keypoints_netout_batch = network_output_heads[1].cpu().numpy()
                    best_gt_keypoints_netout_batch = sample[
                        "keypoint_projections_output"
                    ]
                    best_net_input_images_viz_batch = sample["image_rgb_input_viz"]
 #                   network_output_heads = dream_network.model(network_image_input)
                    best_w2d = network_output_heads[2].double().cuda()
                    best_w2d = best_w2d.view(n_outliers, -1, 2)
                    best_log_weight_scale = dream_network.model.module.log_weight_scale
                    best_log_weight_scale = best_log_weight_scale.cuda()
                    best_w2d = (best_w2d.log_softmax(dim=-2) + best_log_weight_scale).exp()
                    best_gt_belief_maps = sample['belief_maps']
...
#                    median_net_input_images_viz_batch = sample["image_rgb_input_viz"]
                    network_output_heads = dream_network.model(network_image_input)
                    median_samples_belief_maps = network_output_heads[0]
                    median_detected_keypoints_netout_batch = network_output_heads[1].cpu().numpy()
                    median_gt_keypoints_netout_batch = sample[
                        "keypoint_projections_output"
                    ]
                    median_net_input_images_viz_batch = sample["image_rgb_input_viz"]
#                    network_output_heads = dream_network.model(network_image_input)
                    median_w2d = network_output_heads[2].double().cuda()
                    median_w2d = median_w2d.view(n_outliers, -1, 2)
                    median_log_weight_scale = dream_network.model.module.log_weight_scale
                    median_log_weight_scale = median_log_weight_scale.cuda()
                    median_w2d = (median_w2d.log_softmax(dim=-2) + median_log_weight_scale).exp()
                    median_gt_belief_maps = sample['belief_maps']
...
#                    worst_net_input_images_viz_batch = sample["image_rgb_input_viz"]
                    network_output_heads = dream_network.model(network_image_input)
                    worst_samples_belief_maps = network_output_heads[0]
                    worst_detected_keypoints_netout_batch = network_output_heads[1].cpu().numpy()
                    worst_gt_keypoints_netout_batch = sample[
                        "keypoint_projections_output"
                    ]
                    worst_net_input_images_viz_batch = sample["image_rgb_input_viz"]
                    worst_w2d = network_output_heads[2].double().cuda()
                    worst_w2d = worst_w2d.view(n_outliers, -1, 2)
                    worst_log_weight_scale = dream_network.model.module.log_weight_scale
                    worst_log_weight_scale = worst_log_weight_scale.cuda()
                    worst_w2d = (worst_w2d.log_softmax(dim=-2) + worst_log_weight_scale).exp()
                    worst_gt_belief_maps = sample['belief_maps']
...
def sample_range_analysis(
    ...
    for (
        ...
        belief_w2d_path = os.path.join(
            output_dir,
            image_prefix
            + "_belief_maps_kp_rank_{}_id_{}.txt".format(sample_rank, sample_name),
        )
        belief_maps_gt_mosaic_path = os.path.join(
            output_dir,
            image_prefix
            + "_belief_maps_gt_rank_{}_id_{}.png".format(sample_rank, sample_name),
        )
        belief_maps_kpimg_mosaic_path = os.path.join(
            output_dir,
            image_prefix
            + "_belief_maps_kpimg_rank_{}_id_{}.png".format(sample_rank, sample_name),
        ) 
        
        with open(belief_w2d_path, 'w') as f:
            f.write(str(sample_w2d))
        ...
        gt_belief_map_images = dream.image_proc.images_from_belief_maps(
            gt_belief_maps, normalization_method=6
        )
        gt_belief_maps_mosaic = dream.image_proc.mosaic_images(
            gt_belief_map_images, rows=2, cols=n_cols, inner_padding_px=10
        )
        gt_belief_maps_mosaic.save(belief_maps_gt_mosaic_path)
        ...
        belief_maps_kpimg_mosaic = dream.image_proc.mosaic_images(
            belief_map_images_kpimg, rows=2, cols=n_cols, inner_padding_px=10
        )
