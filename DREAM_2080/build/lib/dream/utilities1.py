...
def load_keypoints(data_path, object_name, keypoint_names):
    ...
    #print(data_path)
    keypoint_data = {"projections": [],"trans":[],"rot_quat":[], 'idx' : [], 'positions' : []}
    ...
    keypoint_data['rot_quat'].append(object_data["quaternion_xyzw"])
    quat = np.array(keypoint_data['rot_quat'][-1])
    quat_mat = quaternion_to_matrix(torch.from_numpy(quat)).double()
    ...
    for kp_name in keypoint_names:
        ...
        #kp_position_wrt_cam = list(np.array(kp_data["location"]) - np.array(camera_data_location))
...
def load_keypoints_infer(data_path, object_name, keypoint_names):
    assert os.path.exists(
        data_path
    ), 'Expected data_path "{}" to exist, but it does not.'.format(data_path)
