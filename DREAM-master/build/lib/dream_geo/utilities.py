...
def find_ndds_seq_data_in_dir(
    ...
    for each_dir in dirlist:
        ...
        if min_num_image_exts == 0 or min_num_image_exts == 1: # 说明啥都没生成好
            continue
...
def load_seq_keypoints(data_path, object_name, keypoint_names):
    assert os.path.exists(
        data_path
    ), 'Expected data_path "{}" to exist, but it does not.'.format(data_path)
    ...
    data = data[0]
    assert (object_name == data['ROBOR NAME'])
    ...
    for idx, kp_name in enumerate(keypoint_names):
        assert (object_keypoints[idx]["Name"] == kp_name), \
        "Expected keypoint '{}' to exist in the datafile '{}', but it does not. \
            Rather, the keypoints are '{}'".format(
            kp_name, data_path, object_keypoints[idx]['Name']
            )
        keypoint_data["idx"].append(kp_name)
        keypoint_data["projections"].append(object_keypoints[idx]["projected_location"])
        keypoint_data["positions_wrt_cam"].append(object_keypoints[idx]["location_wrt_cam"])
        
        x3d_wrt_cam = np.array(object_keypoints[idx]["location_wrt_cam"])
...
def get_hm(kp_projs_net_output, output_w, output_h):
    n_kp, _ = kp_projs_net_output.shape
    gt_hm = np.zeros((n_kp, output_h, output_w), dtype=np.float32)
    radius = 4
    for i in range(n_kp):
        ct = deepcopy(kp_projs_net_output[i])
