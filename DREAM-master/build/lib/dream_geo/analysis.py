...
def analyze_ndds_dataset(
    ...
    with torch.no_grad():
        ...
        for batch_idx, sample in enumerate(tqdm(data_loader)):
            ...
            network_x3d_input = sample["keypoint_positions"].cuda()
            network_name = sample["config"]["name"]
            _, detected_keypoints_netout_batch = dream_network.inference(
                network_image_input, network_x3d_input,network_name
            )
        ...
        if visualize_belief_maps:
            ...
            with torch.no_grad():
                for batch_idx, sample in enumerate(best_samples_data_loader):
                    ...
                    network_x3d_input = sample["keypoint_positions"].cuda()
                    network_name = sample["config"]["name"]
                    network_output = dream_network.inference(network_image_input, network_x3d_input,network_name)
...
def analyze_ndds_center_dream_dataset(
    ...
    with torch.no_grad():
        ...
        for idx, sample in enumerate(zip(json_list, detected_kp_proj_list)):
            ...
            # 导入json文件
            gt_kps_raw,  gt_kps_pos, gt_kps_blend = [], [], []
