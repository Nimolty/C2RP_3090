...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--dataset', default="/mnt/data/Dream_ty/franka_data_0825/",
                             help='see lib/dataset/dataset_facotry for ' + 
                            'available datasets')
    self.parser.add_argument('--root_dir', type=str, default="/mnt/data/Dream_ty/Dream_model/center-dream/")
    ...
    self.parser.add_argument('--arch_config', type=str, default="/home/zekaiyin/summer_ty/DREAM-master/arch_configs/dream_vgg_q.yaml")
  ...
  def parse(self, args=''):
    ...
    opt.exp_dir = os.path.join(opt.root_dir, opt.task)
    opt.save_dir = os.path.join(opt.exp_dir, opt.exp_id)
    # opt.debug_dir = os.path.join(opt.save_dir, 'debug')
    
    if opt.resume and opt.load_model == '':
      opt.load_model = os.path.join(opt.save_dir, 'ckpt', opt.model_last_pth)
    return opt


  def update_dataset_info_and_set_heads(self, opt, num_categories, resolution):
  ...
  def init_infer(self, num_categories, default_resolution, args=''):
    # only used in demo
    opt = self.parse()
    opt = self.update_dataset_info_and_set_heads(opt, num_categories, default_resolution)
    return opt
